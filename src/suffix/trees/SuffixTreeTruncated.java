...
public class SuffixTreeTruncated {
	...
	private static final int RULE_TRUNC = 5;
	
	public SuffixTreeTruncated(List<String> sequences, int maxDepth)
	{
...
//					System.out.println(currentHandleLoc);
					if(false == matched)
					{
						currentSearchLoc = find(k, j, i-1, currentHandleLoc, (j>jL && i-j+1 <= maxDepth));		
					}
//					System.out.println(currentSearchLoc);// + " " + currentSearchLoc.getClosestParent().getSuffixLink());
//					printPath(currentSearchLoc.getClosestParent());
					matched = false;	
...
//					}
					...
					{
						extensionInfo = new ExtensionObject(currentSearchLoc.getClosestParent(), currentSearchLoc.getSteps(), currentSearchLoc.getFirstChar(), RULE_TRUNC, null);
					}
					if(i-j+1 >= maxDepth && false == extensionInfo.getClosestParent().isLeaf() && extensionInfo.getSteps() > 0)
					{
						if(false == (i-j+1 > maxDepth && rule3))
						{
							extensionInfo.getClosestParent().getChild(extensionInfo.getChar()).addAdditionalInfo(j, k);
						}
					}
					else if(extensionInfo.getClosestParent().isLeaf() || 
							(i-j+1 >= maxDepth && false == extensionInfo.getClosestParent().isLeaf() && extensionInfo.getSteps() == 0))
					{
						if(false == (i-j+1 > maxDepth && rule3))
						{
	...
	//						System.out.println("setting suffix link!");
					...
					if(extensionInfo.getRule() == RULE_3)
					{
						rule3 = true;
					}
					else
...
//		System.out.println("inside ***find***");
		...
		LocationPointer startLoc = prevHandleLoc;
//		System.out.println("start location: " + startLoc);
		if(end-start+2 < maxDepth)
		{
			startLoc = stepup(prevHandleLoc);
		}
//		System.out.println("start location: " + startLoc);
		SuffixNode startNode = startLoc.getClosestParent();
		int gamaStrInd = -1; //??
		int gamaStart = -1;
		int gamaEnd = -1;
		if(false == startNode.isRoot() && startLoc.getSteps() > 0)
		{
...
//			System.out.println(sv + " " + gamaStart + " " + gamaEnd);
			if(false == sv.isRoot())
			{
...
//		System.out.println("gamaStart " + gamaStart + " gamaEnd " + gamaEnd);
	...
	public LocationPointer stepup(LocationPointer loc)
	{
		...
		{
			System.out.println(loc + " Should not step up from the root");
		}
		if(steps > 0)
		{
...
//			System.out.println(inEdge.getStartInd() + " " + inEdge.getEndInd());
...
//			System.out.println(s + " start = " + start + " end = " + end);
	...
	public void printPath(SuffixNode node)
	{
		String s = "";
		while(node != root)
		{
			s = getSubstring(node.getInEdge().getStrInd(), node.getInEdge().getStartInd(), node.getInEdge().getEndInd()) + s;
			node = node.getParent();
		}
		System.out.println(s);
	}
	
	public String printPath1(SuffixNode node)
	{
		String s = "";
		while(node != root)
		{
	...
	public void finalize(SuffixNode node, int depth)
	{
		if(depth >= maxDepth)
		{
			if(false == node.isLeaf())
			{
				HashMap<Integer, List<Integer>> positions = extractSequencesIndices(node, null);
				node.setAdditionalInfo(positions);
				node.setChildren(null);
			}
			if(depth > maxDepth)
			{
				SuffixEdge inEdge = node.getInEdge();
				inEdge = new SuffixEdge(inEdge.getStrInd(), inEdge.getStartInd(), inEdge.getEndInd() + maxDepth - depth);
				node.setInEdge(inEdge);
			}
		}
		if(node.isLeaf())
		{
		...
		for(Character c : node.getChildrenCharacters())
		{
			...
			SuffixEdge inEdge = child.getInEdge();
			finalize(child, depth + inEdge.getEndInd() - inEdge.getStartInd() + 1);
		}
	}
	
	public HashMap<Integer, List<Integer>> extractSequencesIndices(SuffixNode node, HashMap<Integer, List<Integer>> positions)
	{
		if(null == positions)
		{
			positions = new HashMap<Integer, List<Integer>>();
		}
		if(null != node.getAdditionalInfo())
		{
			...
			for(int i : containingSequencesInd)
			{
				...
				{
					positions.get(i).addAll(node.getAdditionalInfo().getPositions().get(i));
				}
				else
				{
		...
		if(node.isLeaf()) return positions;
		
		for(char c : node.getChildrenCharacters())
		{
			SuffixNode child = node.getChild(c);
			positions = extractSequencesIndices(child, positions);
		}
		return positions;
	}
...
//		sequences.add("banana");
		...
		SuffixTreeTruncated t = new SuffixTreeTruncated(sequences, 3);
